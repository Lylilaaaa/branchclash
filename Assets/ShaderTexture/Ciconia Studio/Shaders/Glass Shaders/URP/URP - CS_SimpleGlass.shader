Shader "Ciconia Studio/CS_Glass/URP/SimpleGlass"
{
	Properties
	{
		[HideInInspector] _EmissionColor("Emission Color", Color) = (1,1,1,1)
		[HideInInspector] _AlphaCutoff("Alpha Cutoff ", Range(0, 1)) = 0.5
		[Space(15)][Header(Global Properties )][Space(10)]
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				float2 uv0_BumpMap = IN.ase_texcoord7.xyz * _BumpMap_ST.xy + _BumpMap_ST.zw;
				float3 tex2DNode2 = UnpackNormalScale( tex2D( _BumpMap, uv0_BumpMap ), _BumpScale );
				...
				float3 Emission = ( Cubmap179 + ( GrabSreenRefraction385 * ( 1.0 - Opacity87 ) ) ).rgb;
				float3 Specular = 0.5;
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float fresnelNode163 = ( -0.05 + 1.0 * pow( 1.0 - fresnelNdotV163, _PowerFresnel ) );
				float4 temp_output_465_0 = ( _ColorFresnel1 * fresnelNode163 * _FresnelStrength );
				float2 uv0_BumpMap = IN.ase_texcoord3.xyz * _BumpMap_ST.xy + _BumpMap_ST.zw;
				float3 tex2DNode2 = UnpackNormalScale( tex2D( _BumpMap, uv0_BumpMap ), _BumpScale );
				float3 NormalmapXYZ170 = tex2DNode2;
